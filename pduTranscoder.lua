...
pduTranscoder = {}
pduTranscoder.__index = pduTranscoder

local pduString = {}
pduString.__index = pduString

local smsPduObject = {}
smsPduObject.__index = smsPduObject

function pduTranscoder.new()
    local self = setmetatable({}, pduTranscoder)
    return self
end

function pduTranscoder.newTxSmsObject()
    local smsPduObj = setmetatable({}, smsPduObject)
    smsPduObj = {
        msgReference=0,
        recipient={
            num  = "",
...
function pduTranscoder.decodePduSms(pduSmsString)
    local pduStr = setmetatable(string, pduString)
    pduStr = pduSmsString
    return pduStr:decodePDU()
end

function pduString:decodeOctet()
    if self:len() < 2 then error("Too short, can't get octet!") end
    return tonumber("0x"..self:sub(1,2)), self:sub(3)
end

function pduString:decodeDecOctets(count)
...
function pduString:decOctets(count)
    return self:sub(1,count*2), self:sub(count*2+1)
end

function pduString:decodePayload(decoding, length)
...
function pduString:decode7bitPayload(length)
    local data = {}
    local prevoctet = 0
    local state = 0
    local octet
...
function pduString:decode8bitPayload(length)
    local data = {}
    local octet = 0
...
function pduString:decode16bitPayload(length)
    local data = {}
    local octe1, octet2
    while self ~= "" and length ~=0 do
...
function pduString:decodeTXmsg(response)
...
function pduString:decodeRXmsg(response)
    response.sender = {}
...
function smsPduObject:encode16bitPayload()
    local response = {}
    local length = 0
...
function smsPduObject:encode7bitPayload()
    local response = {}
    local state = 0
    local carryover = 0
    local length = 0
...
function smsPduObject:encodePayload(alphabetOverride)
    local content = self.msg.content
    local length  = 0
...
function pduTranscoder:encodePDUsimple(recipientNumber, content)
...
print(json.encode(pduTranscoder.decodePduSms(pduMSG)))
